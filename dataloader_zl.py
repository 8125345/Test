...
def main():
    # 标注数据目录
    base_image_dir = '/home/zhaoliang/yolov5/mf_fire/images'
    base_xml_dir = '/home/zhaoliang/yolov5/mf_fire/xml_total'
    # base_dir = '../datasets/label_data'
    files = []
    travel_path(base_image_dir, files)

    # 拆分数据集
    train_data, val_data = split_data(files)
    # 创建存放label及训练文件
    label_dir = '/home/zhaoliang/yolov5/mf_fire/labels/'
    os.makedirs(label_dir, exist_ok=True)
    # print(len(files), len(train_data), len(val_data))
    print(len(files))
    # 记录训练集
    with open('/home/zhaoliang/yolov5/mf_fire/train.txt', 'w') as fp:
        # for i, item in enumerate(train_data):
        for i, item in enumerate(train_data):
            print("train  --->  begin process id:{}, path:{}".format(i, item))
            train_xml_path = item.replace(base_image_dir,base_xml_dir).replace('.jpg', '.xml')
            train_label_path = item.replace(base_image_dir,label_dir).replace('.jpg', '.txt')
            convert_annotation(train_xml_path, train_label_path)
            fp.write(item + '\n')

    # 记录验证集
    with open('/home/zhaoliang/yolov5/mf_fire/test.txt', 'w') as fp:
        for i, item in enumerate(val_data):
            val_xml_path = item.replace(base_image_dir,base_xml_dir).replace('.jpg', '.xml')
            print("val --->   begin process id:{}, path:{}".format(i, item))
            val_label_path = item.replace(base_image_dir,label_dir).replace('.jpg', '.txt')
