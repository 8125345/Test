...
def draw(ch_data, mask_data):
    ...
    chen_y = [ch_data[k] for k in label_new]
    mask_y = [mask_data[k] for k in label_new]

    # 占位以免 数据源标签丢失
    y0 = ["" for i in range(len(label_new))]
    plt.bar(label_new_x, chen_y, alpha=0.7, width=0.2, color='r', label="chen", tick_label=y0)
    plt.bar(label_new_x1, mask_y, alpha=0.7, width=0.2, color='b', label="mask", tick_label=label_new)
    # 给图加text
    for x, y in zip(label_new_x, chen_y):
        plt.text(x, y + 0.05, '%.2f' % y, ha='center', va='bottom', fontsize='7')
    for x, y in zip(label_new_x, mask_y):
    ...
    plt.title(title)  # 图标题
    plt.legend()
    os.makedirs('data/charts_comcla',exist_ok=True)
    img_path = os.path.join('data/charts_comcla',title+'.jpg')
...
if __name__ == '__main__':
    with open('result_combine_class.json','r',encoding='utf8') as fp:
